                ...
                <tbody>
                    ...
                    foreach ($sponsors as $sponsor) : ?>
                        <tr>
                            ...
                            <td><img src="<?= base_url('assets/images/sponsors/' . $sponsor['photo_url']); ?>" width="100px" height="100px" alt="<?= $sponsor['photo_alternate']; ?>"></td>
                            <td><?= $sponsor['last_modified_by']; ?></td>
                            <td>
                                <form action="<?= base_url('admin/sponsors/delete/' . $sponsor['uuid']); ?>" method="POST" class="d-inline">
                                    <?= csrf_field(); ?>
                                ...
                                <a id="<?= $sponsor['uuid']; ?>" class="btn btn-primary view_data"><i class="bi bi-pencil-square"></i></a>
                            </td>
                        </tr>
                    <?php
                        $i++;
    ...
    <div class="modal-dialog modal-dialog-centered" role="document" id="data_add">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addLabel">Add Sponsor</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action='<?= base_url("admin/sponsors/add"); ?>' method="POST" enctype="multipart/form-data">
                    <?= csrf_field(); ?>
...
<script defer>
    $(document).ready(function() {
        $('body').on("click", ".view_data", function(event) {
            ...
            // memulai ajax
            $.ajax({
                url: '<?= base_url('admin/sponsors/detail'); ?>' + '/' + uuid,
                method: 'get',
